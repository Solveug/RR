...
module Translit
  def self.convert!(text, enforce_language = nil)
    ...
    map = send(language.to_s).sort_by { |k, v| v.length <=> k.length }
  ...
  def self.create_russian_map
    english.each_with_object({}) do |tuple, acc|
  ...
  def self.english
    { 'a' => %w[А а], 'b' => %w[Б б], 'v' => %w[В в], 'g' => %w[Г г], 'd' => %w[Д д], 'e' => %w[Е е], 'yo' => %w[Ё ё], 'jo' => %w[Ё ё], 'ö' => %w[Ё ё], 'zh' => %w[Ж ж],
      'z' => %w[З з], 'i' => %w[И и], 'j' => %w[Й й], 'k' => %w[К к], 'l' => %w[Л л], 'm' => %w[М м], 'n' => %w[Н н], 'o' => %w[О о], 'p' => %w[П п], 'r' => %w[Р р],
      's' => %w[С с], 't' => %w[Т т], 'u' => %w[У у], 'f' => %w[Ф ф], 'h' => %w[Х х], 'x' => %w[Кс кс], 'ts' => %w[Ц ц], 'ch' => %w[Ч ч], 'sh' => %w[Ш ш], 'w' => %w[В в],
      'shch' => %w[Щ щ], 'sch' => %w[Щ щ], '#' => %w[Ъ ъ], 'y' => %w[Ы ы], '' => %w[Ь ь], 'je' => %w[Э э], 'ä' => %w[Э э], 'yu' => %w[Ю ю], 'ju' => %w[Ю ю],
